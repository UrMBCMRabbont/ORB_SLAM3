...
cv::Mat System::TrackStereo_cv(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    cv::Mat Tcw = mpTracker->GrabImageStereo_cv(imLeftToFeed,imRightToFeed,timestamp,filename);
